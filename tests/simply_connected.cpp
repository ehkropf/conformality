...
protected:
    void SetUp() override
    {
        ...
        std::vector<ComplexDouble> squarePoints = {
            ComplexDouble(1.0, 1.0),
            ComplexDouble(-1.0, 1.0),
            ComplexDouble(-1.0, -1.0),
            ComplexDouble(1.0, -1.0)
        };
...
TEST_F(SimplyConnectedDomainTest, InternalCircularDomainContainment)
{
    ...
    EXPECT_TRUE(domain.contains(ComplexDouble(0.0, 0.0)));     // Origin
    EXPECT_TRUE(domain.contains(ComplexDouble(0.5, 0.0)));     // On x-axis
    EXPECT_TRUE(domain.contains(ComplexDouble(0.0, 0.5)));     // On y-axis
    EXPECT_TRUE(domain.contains(ComplexDouble(0.3, 0.4)));     // General interior point
    EXPECT_TRUE(domain.contains(ComplexDouble(-0.2, -0.3)));   // Negative quadrant

    // Test points clearly outside the unit circle
    EXPECT_FALSE(domain.contains(ComplexDouble(2.0, 0.0)));    // Far on x-axis
    EXPECT_FALSE(domain.contains(ComplexDouble(0.0, 2.0)));    // Far on y-axis
    EXPECT_FALSE(domain.contains(ComplexDouble(1.5, 1.5)));    // Far in positive quadrant
    EXPECT_FALSE(domain.contains(ComplexDouble(-2.0, -2.0)));  // Far in negative quadrant

    // Test points near the boundary (should be inside for internal domain)
    EXPECT_TRUE(domain.contains(ComplexDouble(0.95, 0.0)));    // Close to boundary on x-axis
    EXPECT_TRUE(domain.contains(ComplexDouble(0.0, 0.95)));    // Close to boundary on y-axis
}
...
TEST_F(SimplyConnectedDomainTest, ExternalCircularDomainContainment)
{
    ...
    EXPECT_FALSE(domain.contains(ComplexDouble(0.0, 0.0)));    // Origin
    EXPECT_FALSE(domain.contains(ComplexDouble(0.5, 0.0)));    // On x-axis
    EXPECT_FALSE(domain.contains(ComplexDouble(0.0, 0.5)));    // On y-axis
    EXPECT_FALSE(domain.contains(ComplexDouble(0.3, 0.4)));    // General interior point

    // Test points clearly outside the unit circle (should be inside external domain)
    EXPECT_TRUE(domain.contains(ComplexDouble(2.0, 0.0)));     // Far on x-axis
    EXPECT_TRUE(domain.contains(ComplexDouble(0.0, 2.0)));     // Far on y-axis
    EXPECT_TRUE(domain.contains(ComplexDouble(1.5, 1.5)));     // Far in positive quadrant
    EXPECT_TRUE(domain.contains(ComplexDouble(-2.0, -2.0)));   // Far in negative quadrant

    // Test points near the boundary from outside
    EXPECT_TRUE(domain.contains(ComplexDouble(1.05, 0.0)));    // Just outside boundary on x-axis
    EXPECT_TRUE(domain.contains(ComplexDouble(0.0, 1.05)));    // Just outside boundary on y-axis
}
...
TEST_F(SimplyConnectedDomainTest, EllipticalDomainContainment)
{
    ...
    EXPECT_TRUE(domain.contains(ComplexDouble(0.0, 0.0)));     // Origin
    EXPECT_TRUE(domain.contains(ComplexDouble(1.0, 0.0)));     // Inside along major axis
    EXPECT_TRUE(domain.contains(ComplexDouble(0.0, 0.5)));     // Inside along minor axis
    EXPECT_TRUE(domain.contains(ComplexDouble(0.5, 0.3)));     // General interior point

    // Test points clearly outside the ellipse
    EXPECT_FALSE(domain.contains(ComplexDouble(3.0, 0.0)));    // Outside along major axis
    EXPECT_FALSE(domain.contains(ComplexDouble(0.0, 2.0)));    // Outside along minor axis
    EXPECT_FALSE(domain.contains(ComplexDouble(2.0, 1.0)));    // Outside in general position

    // Test points near the boundary
    EXPECT_TRUE(domain.contains(ComplexDouble(1.8, 0.0)));     // Near major axis endpoint
    EXPECT_TRUE(domain.contains(ComplexDouble(0.0, 0.9)));     // Near minor axis endpoint
}
...
TEST_F(SimplyConnectedDomainTest, DiscreteBoundaryContainment)
{
    ...
    EXPECT_TRUE(domain.contains(ComplexDouble(0.0, 0.0)));     // Origin
    EXPECT_TRUE(domain.contains(ComplexDouble(0.5, 0.5)));     // Interior point
    EXPECT_TRUE(domain.contains(ComplexDouble(-0.5, 0.5)));    // Another interior point

    // Test points clearly outside the square
    EXPECT_FALSE(domain.contains(ComplexDouble(2.0, 0.0)));    // Outside right
    EXPECT_FALSE(domain.contains(ComplexDouble(0.0, 2.0)));    // Outside top
    EXPECT_FALSE(domain.contains(ComplexDouble(-2.0, -2.0)));  // Outside bottom-left

    // Test points near the boundary
    EXPECT_TRUE(domain.contains(ComplexDouble(0.9, 0.9)));     // Close to corner
    EXPECT_TRUE(domain.contains(ComplexDouble(0.0, 0.9)));     // Close to edge
}
...
TEST_F(SimplyConnectedDomainTest, BoundaryTransformation)
{
    ...
    auto scaleTransform = [](const ComplexDouble& z) -> ComplexDouble
    {
        return z * ComplexDouble(2.0, 0.0);
    };

    // Get some points on the original boundary
    auto originalSamples = domain.getBoundary().sample(8);

    // Verify original points are on unit circle
    for (const auto& point : originalSamples)
    {
        EXPECT_NEAR(1.0, point.abs(), 1e-10);
    }
    ...
    auto transformedSamples = domain.getBoundary().sample(8);

    // Verify transformed points are on circle of radius 2
    for (const auto& point : transformedSamples)
    {
        EXPECT_NEAR(2.0, point.abs(), 1e-1); // Relaxed tolerance due to discrete approximation
    }

    // Test containment after transformation
    EXPECT_TRUE(domain.contains(ComplexDouble(1.5, 0.0)));     // Inside scaled circle
    EXPECT_TRUE(domain.contains(ComplexDouble(0.5, 0.0)));     // Also inside
}
...
TEST_F(SimplyConnectedDomainTest, ComplexTransformation)
{
    ...
    auto squareTransform = [](const ComplexDouble& z) -> ComplexDouble
    {
        return z * z;
    };
...
TEST_F(SimplyConnectedDomainTest, EdgeCasesAndRobustness)
{
    ...
    EXPECT_TRUE(domain.contains(ComplexDouble(1.0, 0.0)));     // Exactly on boundary
    EXPECT_TRUE(domain.contains(ComplexDouble(0.0, 1.0)));     // Exactly on boundary
    EXPECT_TRUE(domain.contains(ComplexDouble(-1.0, 0.0)));    // Exactly on boundary
    ...
    ComplexDouble testPoint = component.evaluate(0.0);
...
TEST_F(SimplyConnectedDomainTest, IdentityTransformation)
{
    ...
    auto identityTransform = [](const ComplexDouble& z) -> ComplexDouble
    {
        return z;
    };
    ...
    EXPECT_TRUE(domain.contains(ComplexDouble(0.0, 0.0)));     // Still inside
    EXPECT_FALSE(domain.contains(ComplexDouble(3.0, 0.0)));    // Still outside
}
